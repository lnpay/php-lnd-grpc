<?php
# Generated by the protocol buffer compiler.  DO NOT EDIT!
# source: lightning.proto

namespace Lnrpc;

use Google\Protobuf\Internal\GPBType;
use Google\Protobuf\Internal\RepeatedField;
use Google\Protobuf\Internal\GPBUtil;

/**
 * Generated from protobuf message <code>lnrpc.WalletBalanceResponse</code>
 */
class WalletBalanceResponse extends \Google\Protobuf\Internal\Message
{
    /**
     * The balance of the wallet
     *
     * Generated from protobuf field <code>int64 total_balance = 1;</code>
     */
    protected $total_balance = 0;
    /**
     * The confirmed balance of a wallet(with >= 1 confirmations)
     *
     * Generated from protobuf field <code>int64 confirmed_balance = 2;</code>
     */
    protected $confirmed_balance = 0;
    /**
     * The unconfirmed balance of a wallet(with 0 confirmations)
     *
     * Generated from protobuf field <code>int64 unconfirmed_balance = 3;</code>
     */
    protected $unconfirmed_balance = 0;
    /**
     * The total amount of wallet UTXOs held in outputs that are locked for
     * other usage.
     *
     * Generated from protobuf field <code>int64 locked_balance = 5;</code>
     */
    protected $locked_balance = 0;
    /**
     * The amount of reserve required.
     *
     * Generated from protobuf field <code>int64 reserved_balance_anchor_chan = 6;</code>
     */
    protected $reserved_balance_anchor_chan = 0;
    /**
     * A mapping of each wallet account's name to its balance.
     *
     * Generated from protobuf field <code>map<string, .lnrpc.WalletAccountBalance> account_balance = 4;</code>
     */
    private $account_balance;

    /**
     * Constructor.
     *
     * @param array $data {
     *     Optional. Data for populating the Message object.
     *
     *     @type int|string $total_balance
     *           The balance of the wallet
     *     @type int|string $confirmed_balance
     *           The confirmed balance of a wallet(with >= 1 confirmations)
     *     @type int|string $unconfirmed_balance
     *           The unconfirmed balance of a wallet(with 0 confirmations)
     *     @type int|string $locked_balance
     *           The total amount of wallet UTXOs held in outputs that are locked for
     *           other usage.
     *     @type int|string $reserved_balance_anchor_chan
     *           The amount of reserve required.
     *     @type array|\Google\Protobuf\Internal\MapField $account_balance
     *           A mapping of each wallet account's name to its balance.
     * }
     */
    public function __construct($data = NULL) {
        \GPBMetadata\Lightning::initOnce();
        parent::__construct($data);
    }

    /**
     * The balance of the wallet
     *
     * Generated from protobuf field <code>int64 total_balance = 1;</code>
     * @return int|string
     */
    public function getTotalBalance()
    {
        return $this->total_balance;
    }

    /**
     * The balance of the wallet
     *
     * Generated from protobuf field <code>int64 total_balance = 1;</code>
     * @param int|string $var
     * @return $this
     */
    public function setTotalBalance($var)
    {
        GPBUtil::checkInt64($var);
        $this->total_balance = $var;

        return $this;
    }

    /**
     * The confirmed balance of a wallet(with >= 1 confirmations)
     *
     * Generated from protobuf field <code>int64 confirmed_balance = 2;</code>
     * @return int|string
     */
    public function getConfirmedBalance()
    {
        return $this->confirmed_balance;
    }

    /**
     * The confirmed balance of a wallet(with >= 1 confirmations)
     *
     * Generated from protobuf field <code>int64 confirmed_balance = 2;</code>
     * @param int|string $var
     * @return $this
     */
    public function setConfirmedBalance($var)
    {
        GPBUtil::checkInt64($var);
        $this->confirmed_balance = $var;

        return $this;
    }

    /**
     * The unconfirmed balance of a wallet(with 0 confirmations)
     *
     * Generated from protobuf field <code>int64 unconfirmed_balance = 3;</code>
     * @return int|string
     */
    public function getUnconfirmedBalance()
    {
        return $this->unconfirmed_balance;
    }

    /**
     * The unconfirmed balance of a wallet(with 0 confirmations)
     *
     * Generated from protobuf field <code>int64 unconfirmed_balance = 3;</code>
     * @param int|string $var
     * @return $this
     */
    public function setUnconfirmedBalance($var)
    {
        GPBUtil::checkInt64($var);
        $this->unconfirmed_balance = $var;

        return $this;
    }

    /**
     * The total amount of wallet UTXOs held in outputs that are locked for
     * other usage.
     *
     * Generated from protobuf field <code>int64 locked_balance = 5;</code>
     * @return int|string
     */
    public function getLockedBalance()
    {
        return $this->locked_balance;
    }

    /**
     * The total amount of wallet UTXOs held in outputs that are locked for
     * other usage.
     *
     * Generated from protobuf field <code>int64 locked_balance = 5;</code>
     * @param int|string $var
     * @return $this
     */
    public function setLockedBalance($var)
    {
        GPBUtil::checkInt64($var);
        $this->locked_balance = $var;

        return $this;
    }

    /**
     * The amount of reserve required.
     *
     * Generated from protobuf field <code>int64 reserved_balance_anchor_chan = 6;</code>
     * @return int|string
     */
    public function getReservedBalanceAnchorChan()
    {
        return $this->reserved_balance_anchor_chan;
    }

    /**
     * The amount of reserve required.
     *
     * Generated from protobuf field <code>int64 reserved_balance_anchor_chan = 6;</code>
     * @param int|string $var
     * @return $this
     */
    public function setReservedBalanceAnchorChan($var)
    {
        GPBUtil::checkInt64($var);
        $this->reserved_balance_anchor_chan = $var;

        return $this;
    }

    /**
     * A mapping of each wallet account's name to its balance.
     *
     * Generated from protobuf field <code>map<string, .lnrpc.WalletAccountBalance> account_balance = 4;</code>
     * @return \Google\Protobuf\Internal\MapField
     */
    public function getAccountBalance()
    {
        return $this->account_balance;
    }

    /**
     * A mapping of each wallet account's name to its balance.
     *
     * Generated from protobuf field <code>map<string, .lnrpc.WalletAccountBalance> account_balance = 4;</code>
     * @param array|\Google\Protobuf\Internal\MapField $var
     * @return $this
     */
    public function setAccountBalance($var)
    {
        $arr = GPBUtil::checkMapField($var, \Google\Protobuf\Internal\GPBType::STRING, \Google\Protobuf\Internal\GPBType::MESSAGE, \Lnrpc\WalletAccountBalance::class);
        $this->account_balance = $arr;

        return $this;
    }

}

